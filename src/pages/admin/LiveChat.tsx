...
import { StaffLoading } from "@/components/staff/StaffConsole";
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className="h-20 w-full" />)}</div>
        <StaffLoading rows={5} />
      </div>
    );
  }
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.map((m, idx) => {
                  ...
                  <div key={m.id} className={`flex flex-col ${m.sender_type === "admin" ? "items-end" : "items-start"}`}>
                    ...
                    {gap && (
                      <span className={`mt-0.5 text-[9px] font-medium ${slow ? "text-warning" : "text-muted-foreground"}`}>
                        {bn ? `উত্তরের ব্যবধান ${gap}` : `gap ${gap}`}
                      </span>
                    )}
                  </div>
                  );
