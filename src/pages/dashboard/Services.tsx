    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">আপনার সব সার্ভিসের তালিকা</p>
        </div>
      </div>
      ...
      {services.length === 0 ? (
        <div className="glass-card p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">কোনো সার্ভিস নেই</h3>
          <p className="text-sm text-muted-foreground mb-4">আপনার এখনো কোনো হোস্টিং সার্ভিস নেই।</p>
          <a href="/#pricing" className="inline-flex items-center gap-2 gradient-primary text-primary-foreground px-6 py-3 rounded-xl font-semibold">
            Browse Plans <ExternalLink className="w-4 h-4" />
          </a>
        </div>
      ) : (
            ...
            <div key={service.id} className="glass-card p-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-xs px-3 py-1 rounded-full font-medium ${statusColors[service.status] || ""}`}>
                    {service.status}
                  </span>
              ...
              {service.expiry_date && (
                <p className="text-xs text-muted-foreground mt-3">
                  Expires: {new Date(service.expiry_date).toLocaleDateString("bn-BD")}
                </p>
              )}
            </div>
          ))}
