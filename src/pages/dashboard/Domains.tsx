...
const DashboardDomains = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("services").select("*").eq("user_id", user.id).eq("service_type", "domain").order("created_at", { ascending: false }).then(({ data }) => { setDomains(data || []); setLoading(false); });
  }, [user]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{tr("dash.domainsTitle")}</h1>
          <p className="text-sm text-muted-foreground">{tr("dash.domainsSubtitle")}</p>
        </div>
      ...
      {domains.length === 0 ? (
        <div className="glass-card p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">{tr("dash.noDomains")}</h3>
          <p className="text-sm text-muted-foreground mb-4">{tr("dash.noDomainsDesc")}</p>
          <a href="/#domain" className="inline-flex items-center gap-2 gradient-primary text-primary-foreground px-6 py-3 rounded-xl font-semibold">
            {tr("dash.searchDomains")} <ExternalLink className="w-4 h-4" />
          </a>
        </div>
      ) : (
            ...
            <div key={d.id} className="glass-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{tr("dash.expires")} {d.expiry_date ? new Date(d.expiry_date).toLocaleDateString("bn-BD") : "N/A"}</p>
                </div>
              </div>
              <span className={`text-xs px-3 py-1 rounded-full font-medium ${d.status === "active" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>{d.status}</span>
            </div>
          ))}
        </div>
      )}
