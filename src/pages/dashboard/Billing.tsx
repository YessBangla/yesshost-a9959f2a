  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">আপনার সব পেমেন্ট এবং ইনভয়েস</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="glass-card p-5">
          ...
          <p className="text-2xl font-bold text-success">
            ৳{invoices.filter(i => i.status === "paid").reduce((s, i) => s + Number(i.amount_bdt), 0).toFixed(2)}
          </p>
        </div>
      </div>
      ...
      {invoices.length === 0 ? (
        <div className="glass-card p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">কোনো ইনভয়েস নেই</h3>
          <p className="text-sm text-muted-foreground">আপনার এখনো কোনো ইনভয়েস নেই।</p>
        </div>
      ) : (
                ...
                {invoices.map((inv) => (
                  <tr key={inv.id} className="border-b border-border/50 hover:bg-secondary/10 transition-colors">
                    ...
                    <td className="p-4 text-muted-foreground">
                      {inv.due_date ? new Date(inv.due_date).toLocaleDateString("bn-BD") : "-"}
                    </td>
                  </tr>
                ))}
              </tbody>
