...
const DashboardOverview = () => {
  ...
  const cards = [
    { title: "Active Services", value: stats.services, icon: Server, color: "text-primary", link: "/dashboard/services" },
    { title: "Pending Invoices", value: stats.invoices, icon: FileText, color: "text-warning", link: "/dashboard/billing" },
    { title: "Support Tickets", value: stats.tickets, icon: HeadphonesIcon, color: "text-success", link: "/dashboard/support" },
    { title: "Domains", value: stats.domains, icon: Globe, color: "text-info", link: "/dashboard/domains" },
  ];
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">আপনার অ্যাকাউন্টের সারসংক্ষেপ</p>
      </div>
        ...
        <div className="glass-card p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link to="/dashboard/support" className="flex items-center gap-2 p-3 rounded-xl bg-secondary/50 hover:bg-secondary text-sm font-medium text-foreground transition-colors">
              <HeadphonesIcon className="w-4 h-4 text-primary" /> Open Ticket
            </Link>
            <Link to="/dashboard/billing" className="flex items-center gap-2 p-3 rounded-xl bg-secondary/50 hover:bg-secondary text-sm font-medium text-foreground transition-colors">
              <FileText className="w-4 h-4 text-primary" /> View Invoices
            </Link>
            <Link to="/dashboard/services" className="flex items-center gap-2 p-3 rounded-xl bg-secondary/50 hover:bg-secondary text-sm font-medium text-foreground transition-colors">
              <Server className="w-4 h-4 text-primary" /> My Services
            </Link>
            <Link to="/dashboard/domains" className="flex items-center gap-2 p-3 rounded-xl bg-secondary/50 hover:bg-secondary text-sm font-medium text-foreground transition-colors">
              <Globe className="w-4 h-4 text-primary" /> My Domains
            </Link>
          </div>
        </div>

        <div className="glass-card p-6">
          <h3 className="text-base font-bold text-foreground mb-4">Recent Invoices</h3>
          {recentInvoices.length === 0 ? (
            <div className="text-center py-8">
              <AlertCircle className="w-10 h-10 text-muted-foreground mx-auto mb-2" />
              <p className="text-sm text-muted-foreground">কোনো ইনভয়েস নেই</p>
            </div>
          ) : (
